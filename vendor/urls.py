...
from .views import (CreateDealOfTheRequestView, DeleteUpdateRetrieveDealOfTheRequestView,\
     FeaturedVendors, ListCreateFlashRequest, VendorCloseOrOpen, VendorList, VendorDetail, ConfirmAccount, VendorLogin, \
        VendorProfile, VendorUpdate)
...
urlpatterns = [
    ...
    path('flashsales/', ListCreateFlashRequest.as_view(), name='flash_sales'),
    path('featured/', FeaturedVendors.as_view(), name='close_open'),
    ...
    path('profile/', VendorProfile.as_view(), name='vendor_dashboard'),
    path('<user__uid>/', VendorDetail.as_view(), name='vendor_detail'),
]